...
- [ ] 1. Extend type definitions for voice layer
  - [ ] 1.1 Add/verify voice types in `src/lib/game/types.ts`
...
- [ ] 2. Implement VOICE_PRESETS constant
  - [ ] 2.1 Create/verify `VOICE_PRESETS` in `src/lib/voice/presets.ts`
  ...
  - [ ] 2.2 Create/verify `PERSONA_VOICE_IDS` in `src/lib/voice/presets.ts`
    - Export `PERSONA_VOICE_IDS: Record<Persona, string>` with placeholder values (TBD_DAY2_TUNING for 3, Reader placeholder from ping-voice)
    - _Requirements: 15.1, 15.2_
    - _Files: `src/lib/voice/presets.ts`_

- [ ] 3. Write VOICE_PRESETS invariant tests
  - [ ] 3.1 Write shape completeness tests in `src/lib/voice/presets.test.ts`
  ...
  - [ ] 3.2 Write Misdirector inversion tests in `src/lib/voice/presets.test.ts`
  ...
  - [ ] 3.3 Write Novice audibility tests in `src/lib/voice/presets.test.ts`
    - **Invariant 3:** `Novice.lying.stability <= 0.25`
    - **Invariant 3:** `Novice.lying.style >= 0.55`
    - _Requirements: 4.1, 4.2_
    - _Files: `src/lib/voice/presets.test.ts`_

  - [ ] 3.4 Write Silent subtlety + Reader ordering tests in `src/lib/voice/presets.test.ts`
    - **Invariant 4:** `|Silent.honest.stability − Silent.lying.stability| < 0.25`
    - **Invariant 5:** Novice delta > Reader delta > Silent delta
    - _Requirements: 5.1, 6.1, 6.2_
    - _Files: `src/lib/voice/presets.test.ts`_

  - [ ] 3.5 Write preset reference stability test in `src/lib/voice/presets.test.ts`
    - **Invariant 6:** Two dynamic imports return the same object reference
    - _Requirements: 7.1_
    - _Files: `src/lib/voice/presets.test.ts`_

  - [ ] 3.6 Write PERSONA_VOICE_IDS shape test in `src/lib/voice/presets.test.ts`
    - Every Persona has a defined, non-empty string entry
    - _Requirements: 15.1, 15.2_
    - _Files: `src/lib/voice/presets.test.ts`_

- [ ] 4. Checkpoint — run presets tests
  - Run `pnpm vitest run src/lib/voice/presets.test.ts` and verify all pass. Ask user if questions arise.

- [ ] 5. Implement computeLieScore and FILLER_REGEX
  - [ ] 5.1 Create/verify `FILLER_REGEX` in `src/lib/voice/heuristic.ts`
  ...
  - [ ] 5.2 Create/verify `computeLieScore` in `src/lib/voice/heuristic.ts`
...
- [ ] 6. Write computeLieScore invariant tests
  - [ ] 6.1 Write range + boundary tests in `src/lib/voice/heuristic.test.ts`
  ...
  - [ ] 6.2 Write weight allocation tests in `src/lib/voice/heuristic.test.ts`
...
- [ ] 7. Checkpoint — run heuristic tests
...
- [ ] 9. Checkpoint — run all voice tests
